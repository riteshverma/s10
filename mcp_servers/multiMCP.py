...
from memory.tool_performance import log_tool_performance, build_tool_performance_entry
import ast
    ...
    async def function_wrapper(self, tool_name: str, *args):
        ...
        if tool_name in self.banned_tools:
            log_tool_performance(build_tool_performance_entry(
                ...
                duration_ms=0.0,
                args_count=len(args),
                error="Tool banned after repeated failures"
            ))
            raise ValueError(f"Tool '{tool_name}' is banned after repeated failures.")
        ...
        try:
            ...
            if status == "error":
                ...
                if self.tool_failure_counts[tool_name] >= 3:
